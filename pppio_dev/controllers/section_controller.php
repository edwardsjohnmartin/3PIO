...
class SectionController extends BaseController{
	...
	public function update_students(){
		//If a section_id wasn't passed in, throw an error
		if (!isset($_GET['id'])){
			return call('pages', 'error');
		}
		...
		//If the user isn't the owner or ta for the section, throw an error
		if (!section::is_owner($section_id, $_SESSION['user']->get_id()) and !section::is_teaching_assistant($section_id, $_SESSION['user']->get_id())){
			return call('pages', 'error');
		}
			...
			if(!empty($postedToken) && isTokenValid($postedToken)){
				...
				if(!isset($intersection) or count($intersection) > 0){
					...
					if(!isset($ta_intersection) or count($ta_intersection) > 0){
						...
						//HACK: The 'students property of the section model will not get filled here. The data will get passed through the $_POST and be filled in the update() function in the section class.
						if($section->is_valid()){
							$section->update();
							add_alert('Successfully updated!', Alert_Type::SUCCESS);
							return redirect('section', 'read&id=' . $section_id);
						}
						else{
							add_alert('Please try again.', Alert_Type::DANGER);
						}
