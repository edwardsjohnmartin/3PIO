...
*/
	...
	foreach($exercises as $exercise_id => $exercise_obj)
	{
		...
		{
			echo '<a href="/?controller=exercise&action=try_it&id=' . $exercise_id . '&lesson_id=' . $lesson->get_id() . '&concept_id=' . $concept_id . '" class="tile btn btn-success" id="exercise-' . $exercise_id . '"><span class="tile-number">' . $i . '</span></a>';
		}
		elseif (!$found_latest)
		{
			echo '<a href="/?controller=exercise&action=try_it&id=' . $exercise_id . '&lesson_id=' . $lesson->get_id() . '&concept_id=' . $concept_id . '" class="tile btn btn-default" id="exercise-' . $exercise_id . '"><span class="tile-number">' . $i . '</span></a>';
			$found_latest = true;
			if($exercise_id == $exercise->get_id())
			{
				$trying_latest = true;
		...
		{
			echo '<a class="tile btn btn-default disabled" id="exercise-' . $exercise_id . '"><span class="tile-number">' . '<span class="glyphicon glyphicon-lock" aria-hidden="true"></span><span class="sr-only">Locked</span>' . '</span></a>';
		}
