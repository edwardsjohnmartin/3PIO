...
def VALIDATE_VAR(desired_var_name, desired_type_str, desired_return, *args):
    ...
    if student_var != None:
        type_str = str(type(student_var)).split("<type '")[1].split("'>")[0]
        if type_str == desired_type_str:
            if (len(args) == 1):
            ...
            return "{0} should be of type {1}, but it is of type {2}.".format(desired_var_name, desired_type_str, type_str)
    else:
